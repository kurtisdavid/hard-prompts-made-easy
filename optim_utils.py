...
def optimize_prompt_loop_with_sd_guidance(
    model, 
    tokenizer, 
    token_embedding, 
    all_target_features, 
    ...
    sd_args = args.sd_args
    alpha = sd_args["alpha"]
    guidance_scale=sd_args["guidance_scale"]
    num_inference_steps=sd_args["num_inference_steps"]
    height=sd_args["height"]
    width=sd_args["width"]
    ...
    for step in range(opt_iters):
        ...
        with torch.no_grad():
            ...
            # Also get the new sd guidance targets...
            # TODO: move hard coded values to config
            sd_images = sd_pipeline(
                decoded_text,
        ...
        cosim_scores = logits_per_image
        loss = 1 - cosim_scores.mean()
        # compute sd guidance term
        sd_guidance_logits_per_image, _ = model.forward_text_embedding(padded_embeds, dummy_ids, sd_target_features)
        cosim_scores_sd = sd_guidance_logits_per_image
        ...
        curr_lr = input_optimizer.param_groups[0]["lr"]
        cosim_scores = cosim_scores.mean().item()
        cosim_scores_sd = cosim_scores_sd.mean().item()
...
def optimize_prompt(
    ...
    """Run prompt inversion training. 

    If sd_pipeline is provided, enables second task for guidance with Stable Diffusion (SD).
    """
    ...
    if not sd_pipeline:
        ...
        learned_prompt = optimize_prompt_loop_with_sd_guidance(
            model,
            tokenizer, 
            token_embedding, 
            all_target_features, 
